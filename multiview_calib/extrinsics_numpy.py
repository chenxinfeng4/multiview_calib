...
def _common_landmarks_np(landmarks1, landmarks2, ids1=None, ids2=None):
    if ids1 is None and ids2 is None:
        ...
        # filter save the non nan values
        ind1_nonnan = ~np.isnan(landmarks1[:,0])
        ind2_nonnan = ~np.isnan(landmarks2[:,0])
        landmarks1 = landmarks1[ind1_nonnan]
        landmarks2 = landmarks2[ind2_nonnan]
        ids1 = ids1[ind1_nonnan]
        ids2 = ids2[ind2_nonnan]

        # filter save the comman ids
        issort = lambda a: (a == np.sort(a)).all()
        ids1 = np.array(ids1)
        ids2 = np.array(ids2)
        assert issort(ids1), "ids1 must be sorted"
        assert issort(ids2), "ids2 must be sorted"
        ids_common, ids1_ind, ids2_ind = np.intersect1d(ids1, ids2, return_indices=True)
        pts1 = landmarks1[ids1_ind]
        pts2 = landmarks2[ids2_ind]
        ids_common = np.where(ids_common)[0]
    return pts1, pts2, ids_common
...
def visualise_epilines(view_tree, relative_poses, intrinsics, landmarks, filenames,
    ...
    for view1, view2 in view_tree: 
        ...
        pts1, pts2, _ = _common_landmarks_np(landmarks[view1],
                                          landmarks[view2])
...
def visualise_epilines_np(view_tree, relative_poses, intrinsics, landmarks, background_imgs,
                       output_path="output/relative_poses/"):
    
    for view1, view2 in view_tree: 
        img1 = background_imgs[view1]
        img2 = background_imgs[view2]
...
def compute_relative_poses(view_tree, intrinsics, landmarks, 
    ...
    for view1, view2 in view_tree:       
        ...
        pts1, pts2, ids_common = _common_landmarks_np(landmarks[view1],
                                                      landmarks[view2])
...
def concatenate_relative_poses(minimal_tree, relative_poses, method='cross-ratios', verbose=2, print_prefix=''):
    ...
    while len(pairs)>0: 
        ...
        for curr_pair in pairs:
            ...
            p3d_adj_com, p3d_com, _ = _common_landmarks_np(p3d_adj, p3d, ids_adj, ids)
...
def visualise_global_registration(global_poses, landmarks_global, ba_poses, ba_points, 
    ...
    for view, calib in global_poses.items():
        if isinstance(filenames[view], str):
